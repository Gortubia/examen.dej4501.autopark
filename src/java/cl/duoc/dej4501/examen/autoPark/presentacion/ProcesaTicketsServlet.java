...
public class ProcesaTicketsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //verifica si se envia un codigo desde la tabla de tickets para eliminar el ticket segun su codigo
        ...
        try {
            ...
            //si no se encuentra el parametro "codigo significa que son datos para post"
        ...
        } else {//si el parametro codigo trae un valor se prrocede a eliminar el ticekt de la lista
            //tomamos el atributo voucherVD que tiene la session para procesar cada ticket y comprobar si codigo existe
            voucherVD = (voucherViewDomain) request.getSession().getAttribute("voucherVD");
            //verificamos que el codigo a eliminar se encuentra en la lista del voucher
            for (TicketsViewDomain ticket1 : voucherVD.getListadoTickets()) {
                //comprovamos si codigo esta en la lista
                if (ticket1.getIdTicket() == codigoborrar) {
                    try {
                        //antes de eliminar el ticket del listado. debemos descontar su monto del total 
                        voucherVD.setTotalVoucher(voucherVD.getTotalVoucher() - ticket1.getMonto());
                        //luego de descontar el monto del ticket a eliminar. lo eliminamos de la lista
                        if (voucherVD.getListadoTickets().remove(ticket1)) {
                            //seteamos el voucherVD a la session y retornamos al index.jsp
        ...
        if (validacion(request)) {
            ...
            if (flag) {
                ...
                if (request.getSession().getAttribute("voucherVD") == null) {
                    ...
                    /**
                     *
                     * @author adolf // verificamos si ya se encuentra el
                     * estacionamiento en la lista para agregar el monto
                     */
                    int montoNuevo = 0;
                    int montoOld = 0;
                    int lastIdTicket = 0;
